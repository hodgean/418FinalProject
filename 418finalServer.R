...
server<-function(input, output, session) {
  ...
  output$plot21<-renderPlot({
    ...
    plot(acf(xdif),main='Auto Correlation of Vector',col='red',lty=2,pch=2,binwidth=0.2)
  ...
  output$plot2<-renderPlot({
    ...
    plot(acf(xdif),main='Auto Correlation of Vector',col='red',lty=2,pch=2)
  ...
  output$plot31<-renderPlot({
    ...
    xdif<-diff(timeseries)
    xdif<-(xdif[!is.na(xdif)])
    plot(pacf(xdif),main='Partial Auto Correlation of Vector',col='red',lty=2,pch=2)
  })
  ...
  output$T5 <- renderPrint({
    ...
    print(paste('Predicted value of magnitude: next 15 minutes ',p))
  ...
  output$E2 <- renderPrint({
    ...
    if(p<3) 
    {
      print("Since the predicted value < 3, no significant earthquake for next 15 minutes")
    }
